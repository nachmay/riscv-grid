...
`ifdef TSMC_INITIALIZE_MEM
  parameter cdeFileInit  = "TS6N16FFCLLSVTA32X32M4FW_initial.cde";
`endif
`ifdef TSMC_INITIALIZE_FAULT
   parameter cdeFileFault = "TS6N16FFCLLSVTA32X32M4FW_fault.cde";
`endif
...
always @(posedge bCLKW) begin
  ...
  if (f_rose_same_time && mode_sram_func) begin
  ...
  else if ($realtime <= Contention_Window_CLKR && mode_sram_func && m_AA == p_AB && ram_wen && p_ram_ren) begin
      // mask all
      if (&m_BWEB) begin
      end
      // open all
...
always @(posedge bCLKR) begin
  ...
  else if ($realtime <= Contention_Window_CLKW && mode_sram_func && p_AA == m_AB && p_ram_wen && ram_ren) begin
      // mask all
      if (&p_BWEB) begin
      end
      // open all
...
TS6N16FFCLLSVTA32X32M4FW_Int_Array #(1,1,W,N,M,MES_ALL) MX (.D({DL}),.BW({BWEBL}),
...
*/
module TS6N16FFCLLSVTA32X32M4FW_Int_Array (D, BW, AW, EN, RDB, AR, Q);
