...
module TS1N16FFCLLSVTA8192X32M8SW (
  CLK,
  CEB,
  WEB,
  A,
...
buf ub_A8 (b_A[8], A[8]);
buf ub_A9 (b_A[9], A[9]);
buf ub_A10 (b_A[10], A[10]);
buf ub_A11 (b_A[11], A[11]);
buf ub_A12 (b_A[12], A[12]);
buf ub_D0 (b_D[0], D[0]);
buf ub_D1 (b_D[1], D[1]);
buf ub_D2 (b_D[2], D[2]);
buf ub_D3 (b_D[3], D[3]);
...
buf ub_iA10 (i_A[10], b_A[10]);
buf ub_iA11 (i_A[11], b_A[11]);
buf ub_iA12 (i_A[12], b_A[12]);
...
//---- RAM Enable
TS1N16FFCLLSVTA8192X32M8SW_RAM_Enable_handle u_RAM_Enable_handle (
...
//---- RAM Core
TS1N16FFCLLSVTA8192X32M8SW_RAM_Core u_RAM_Core (
  //---- clock
  .RAM_CLK      ( ram_clk),
  //---- RAM IO
  .RAM_D        ( i_D),
...
module TS1N16FFCLLSVTA8192X32M8SW_RAM_D_handle ( MEM, D, BWEB, Z );
...
module TS1N16FFCLLSVTA8192X32M8SW_RAM_Enable_handle (
  MODE_SRAM_OP,
  RAM_WEB,
  RAM_CEB,
  RAM_WRITE,
...
module TS1N16FFCLLSVTA8192X32M8SW_RAM_Core (
  //---- clock
  RAM_CLK,
  //---- RAM IO
  RAM_D,
...
//-------- Combinational Network --------//
//---- input data handling
TS1N16FFCLLSVTA8192X32M8SW_RAM_D_handle u_RDH0 (.MEM(data_mem), .D(RAM_D), .BWEB(RAM_BWEB), .Z(data_in));
